      ...
      <AnimatePresence>
        {status.success ? (
          ...
          <motion.div
            ...
            className="w-full max-w-xs sm:max-w-md bg-white/5 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/10 p-6 sm:p-8"
          >
            <div className="text-center mb-5 sm:mb-6">
              <Shield className="h-10 w-10 text-blue-300 mx-auto mb-2 sm:mb-3" />
              <h1 className="text-xl sm:text-2xl font-bold text-white">Enter Verification Code</h1>
              <p className="text-gray-300 text-xs sm:text-sm mt-1">
                A 6-digit code was sent to <span className="font-medium text-blue-400">{email}</span>
              </p>
            </div>

            <div className="flex justify-center mb-5 sm:mb-6"><CountdownCircle timeLeft={timeLeft} /></div>

            <OtpInput value={otp} onChange={setOtp} />

            {status.error && <p className="text-red-400 text-sm text-center mt-3">{status.error}</p>}

            <button
              onClick={handleVerify}
              disabled={status.loading || otp.length !== 6}
              className="w-full mt-5 sm:mt-6 bg-gradient-to-r from-cyan-400 via-cyan-600 to-cyan-600 text-black py-3 rounded-xl font-medium disabled:opacity-50 transition"
            >
              {status.loading ? <Loader className="animate-spin h-5 w-5 mx-auto" /> : 'Verify'}
            </button>

            <div className="flex justify-between items-center mt-4 sm:mt-5 text-sm">
              <button onClick={onBack} className="text-gray-400 hover:text-white flex items-center gap-1">
                <ArrowLeft className="h-4 w-4" /> Back
              </button>
              <button
                onClick={handleResend}
                disabled={status.resending || timeLeft > 540}
                className="text-blue-400 hover:text-white disabled:opacity-50 flex items-center gap-1"
              >
                {status.resending ? <Loader className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />} Resend
              </button>
            </div>
          </motion.div>
        )}
