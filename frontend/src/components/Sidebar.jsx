...
import { Home, Upload, FileText, BarChart3, Shield, LogOut, Brain, ChevronLeft, ChevronRight, User as UserIcon, X , ShieldCheck, ClipboardCheck, LayoutDashboard, ListOrdered, TrendingUp} from 'lucide-react';

const navigationItems = [
  { id: 'overview', name: 'Dashboard', icon:LayoutDashboard },
  { id: 'upload', name: 'Upload', icon: Upload },
  { id: 'records', name: 'My Logs', icon: ListOrdered },
  { id: 'analytics', name: 'Analytics', icon: TrendingUp, adminOnly: true },
  { id: 'admin', name: 'Admin Console', icon: ShieldCheck, adminOnly: true },
  { id: 'compliance', name: 'Compliance', icon: ClipboardCheck, adminOnly: true },
];

const NavItem = memo(({ item, isActive, isCollapsed, onClick }) => (
    <button onClick={onClick} aria-label={item.name} aria-current={isActive ? 'page' : undefined} className={`w-full text-left rounded-lg flex items-center transition-all duration-200 group relative ${isActive ? 'bg-cyan-400/40 text-white' : 'text-white-300 hover:bg-gray-700/50 hover:text-white'} ${isCollapsed ? 'justify-center py-3' : 'px-4 py-2.5'}`}>
...
const UserProfile = memo(({ user, handleLogout, isCollapsed }) => (
    <div className={`p-4 border-t border-gray-700/50 ${isCollapsed ? 'space-y-4' : ''}`}>
        <div className="flex items-center gap-3">
         ...
         <button onClick={handleLogout} className={`w-full mt-4 flex items-center justify-center gap-2 text-sm font-medium bg-cyan-400/50 hover:bg-cyan-500/20 text-white-300 rounded-lg transition-colors ${isCollapsed ? 'p-2' : 'px-3 py-2'}`}>
...
const Sidebar = ({ user, currentView, onNavigate, onLogout, isCollapsed, onCollapse, isMobileOpen, onMobileClose }) => {
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white backdrop-blur-xl border-r border-gray-700/50 flex flex-col transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'} transform ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
          <div className={`flex items-center justify-between border-b border-gray-700/50 flex-shrink-0 ${isCollapsed ? 'p-4 h-[69px]' : 'p-5'}`}>
              <div className="flex items-center">
                  <div className="bg-gradient-to-r from-cyan-500 to-cyan-800 p-2 rounded-lg shadow-md"><Shield className="h-6 w-6 text-cyan " /></div>
                  {!isCollapsed && <h1 className="text-xl font-bold text-white-100 tracking-tight ml-3">KYC Shield</h1>}
              </div>
