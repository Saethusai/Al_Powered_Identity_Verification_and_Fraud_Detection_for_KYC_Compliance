...
class AdvancedFraudDetector:
    ...
    def analyze_document(
        ...
        if user_entered_name and extracted_fields.get("name"):
            ...
            if similarity_score >= 85:
             ...
             # --- Apply name penalty to manipulation score ---
            name_penalty = 0
            if name_matching_result["match_status"] == "low":
                name_penalty = 20
            elif name_matching_result["match_status"] == "partial":
                name_penalty = 10

            manipulation_result["manipulation_score"] = min(100.0, manipulation_result["manipulation_score"] + name_penalty)
            if name_penalty > 0:
                manipulation_result["detected_issues"].append(
            f"Name mismatch: {user_entered_name} vs {extracted_fields.get('name')}"
        )
    # ------------------------------------------------
    ...
    def detect_document_authenticity(
        ...
        try:
            logger.info(
                f"🔍 Starting authenticity detection for {document_type}: {image_path}"
            )
            ...
            logger.info(
                f"✅ Authenticity detection completed: {result['manipulation_score']}% risk, {result['risk_level']} level"
            )
            return result
    ...
    def _generate_ai_insights(self, detection_result: Dict[str, Any]) -> List[str]:
        ...
        if score >= 80:
            insights.append(
                "🚨 High probability of document manipulation - immediate review recommended"
            )
        elif score >= 60:
            insights.append(
                "⚠️ Moderate manipulation risk - additional verification suggested"
            )
        elif score >= 30:
        ...
        if any("compression" in issue.lower() for issue in detected_issues):
            insights.append(
                "💾 Compression artifacts suggest possible digital manipulation"
            )

        if any("edge" in issue.lower() for issue in detected_issues):
            insights.append(
                "📐 Edge inconsistencies may indicate copy-paste operations"
            )
        ...
        if any(
            ...
            insights.append(
                "🔤 Text rendering anomalies suggest possible field modification"
            )
        ...
        if any(
            ...
            insights.append(
                "💡 Lighting inconsistencies may indicate composite image creation"
            )

        if any("color" in issue.lower() for issue in detected_issues):
            insights.append(
                "🌈 Color distribution anomalies suggest digital processing"
            )

        # Confidence-based insights
        if confidence < 50:
            insights.append(
                "❓ Low analysis confidence - consider manual expert review"
            )
        elif confidence > 85:
        ...
        if score >= 70 and confidence >= 60:
            insights.append(
                "🔍 Strong recommendation: Flag for manual expert verification"
            )
        elif score >= 40 and confidence >= 70:
            insights.append(
                "📋 Moderate recommendation: Additional document validation advised"
            )
